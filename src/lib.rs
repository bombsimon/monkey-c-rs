...
fn parse_value(pair: Pair<Rule>) -> Ast {
    match pair.as_rule() {
        ...
        Rule::import => Ast::Import(Box::new(parse_value(pair.into_inner().next().unwrap()))),
        Rule::using => {
        ...
        Rule::container => {
            ...
            for pair in pairs {
                contains.push(parse_value(pair));
            }

            Ast::Container { name, contains }
        }
        Rule::function => {
            let mut pairs = pair.into_inner();
            ...
            let returns = match pairs.peek().unwrap().as_rule() {
                Rule::alias => Some(Box::new(parse_value(pairs.next().unwrap()))),
                _ => None,
            };
        ...
        Rule::declare => {
            ...
            Ast::Declare { visibility, target }
        }
        Rule::new => {
            let mut pairs = pair.into_inner();
            Ast::New {
                target: Box::new(parse_value(pairs.next().unwrap())),
            }
        }
        Rule::call => {
            let mut pairs = pair.into_inner();
