...
mod tests {
    ...
    #[test]
    fn coverage_test_parses_out_format_and_out() {
        let cli = Cli::try_parse_from([
            "rafiki",
            "coverage",
            "test",
    ...
    #[test]
    fn coverage_test_defaults_out_format_to_text_and_stdout() {
        let cli =
            Cli::try_parse_from(["rafiki", "coverage", "test", "-d", "fr965", "-y", "key.der"])
                .expect("parses");
        let Command::Coverage(CoverageCommand::Test(args)) = cli.command else {
            panic!("expected coverage test");
        };
    ...
    #[test]
    fn coverage_test_parses_instrument_out_separately_from_out() {
        let cli = Cli::try_parse_from([
            "rafiki",
            "coverage",
            "test",
        ...
        assert_eq!(args.instrument_out, Some(PathBuf::from("bin/cov")));
        assert_eq!(args.out, Some(PathBuf::from("coverage.info")));
    }
}
