...
fn run_test(renderer: &Renderer, args: &CoverageTestArgs) -> io::Result<bool> {
    ...
    // Only the first `--jungle` gets rewritten; any further ones are handed to
    // `monkeyc -f` unmodified, in order.
    let mut jungle_arg = out.join("coverage.jungle").display().to_string();
    for extra in args.jungle.iter().skip(1) {
        jungle_arg.push(';');
        jungle_arg.push_str(&extra.display().to_string());
    }
...
fn run_instrument(renderer: &Renderer, args: &CoverageInstrumentArgs) -> io::Result<PathBuf> {
    ...
    let jungles = jungle_paths(&args.jungle, &root);
    let jungle = &jungles[0];
    ...
    // How far `coverage_jungle` needs to rebase the original's relative paths.
    let to_root = relative_path(&out, &root);
    let coverage_jungle_source = coverage_jungle(&jungle_source, &to_root.to_string_lossy())
        .map_err(|e| {
...
/// Resolve `--jungle` into at least one path: explicit values used exactly as
/// given, so they resolve the same way `monkeyc` itself would (relative to
/// the working directory), or `{root}/monkey.jungle` when none were passed.
fn jungle_paths(jungle: &[PathBuf], root: &Path) -> Vec<PathBuf> {
    if jungle.is_empty() {
        vec![root.join("monkey.jungle")]
    } else {
        jungle.to_vec()
    }
}
